...
class AttnGRU:
    ...
    def __call__(self, inputs, state, attention):
        with tf.variable_scope('AttnGRU'):
            r = fully_connected(tf.concat([inputs, state], 1), self.num_units, 'AttnGRU_fc1', init_b=1.0, group_id=1)
            r = batch_norm(r, 'AttnGRU_bn1', self.is_train, self.bn, 'sigmoid')

            c = fully_connected(tf.concat([inputs, r*state], 1), self.num_units, 'AttnGRU_fc2', init_b=0.0, group_id=1)
